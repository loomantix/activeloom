...
Run every command, test, and review lane in the foreground so this turn waits for each to finish: the session ends when this turn ends and discards unfinished background work, so end the turn only after the canonical result is written."
