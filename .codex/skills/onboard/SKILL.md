---
name: onboard
description: Draft the project-specific values a fresh `.activeloom-config.yml` leaves blank — the stack table, code rules, and review focus — by reading the repository, then present them for confirmation. Use after `npx activeloom init` when .activeloom-config.yml still has TODO(activeloom) markers to fill.
---
...
If the file exists and contains no `TODO(activeloom):` markers, say that it is already filled in and stop. Do not rewrite values a human chose.
