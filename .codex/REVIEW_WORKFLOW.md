...
One non-author reviewer is the recommended floor, and covers the great
majority of changes; a single second-model pass (via `pr-critique` or `--chain <reviewer>`)
is the standard Lean default. Cyclic relay (`--cycle --until-converged`) is strictly
reserved for Deep tier changes.
