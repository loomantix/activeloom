...
AGENT_LOOP = REPO_ROOT / ".agents/skills/agent-loop/scripts/agent-loop.sh"
...
def test_uncertain_finalized_checkpoint_restores_resumable_finalizing_state(
    ...
    helper = (
        consumer[0]
        / ".agents/skills/agent-loop/scripts/agent-loop-state.py"
    )
...
def test_default_shipped_prompt_does_not_depend_on_gh() -> None:
    ...
    prompt = (REPO_ROOT / ".agents/skills/agent-loop/prompt.txt.template").read_text(
        encoding="utf-8"
    )
    ...
    instructions = (
        REPO_ROOT / ".agents/skills/agent-loop/agent-loop-instructions.md.template"
    ).read_text(encoding="utf-8")
...
def test_batch_iteration_cap_pauses_cleanly_and_only_one_resume_process_owns_it(
    ...
    config_path = consumer[0] / ".agents/skills/agent-loop/agent-loop.config"
    config_path.write_text(blocking_config, encoding="utf-8")
    env = _agent_loop_env(consumer, issues=[_issue(73)])
    command = [
        str(consumer[0] / ".agents/skills/agent-loop/scripts/agent-loop.sh"),
        "--resume-batch",
        str(batch_file),
    ]
...
def test_batch_cursor_issue_cannot_be_skipped_for_a_later_ready_issue(
    ...
    helper = consumer[0] / ".agents/skills/agent-loop/scripts/agent-loop-state.py"
