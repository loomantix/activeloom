...
Take the identifier from the user, or from a command they run — `agy` and the other engine CLIs can report what they are configured for. Never supply one from memory, and never "fill in a sensible default".
