...
Use `.agents/skills/critique/scripts/review-ledger.py` for every local-review
...
```bash
python3 .agents/skills/critique/scripts/review-ledger.py preflight-anchor \
  --repo <owner/repo> --pr <number> --head <full-head-sha> \
  --path <repository-relative-path> --line <right-side-line>

python3 .agents/skills/critique/scripts/review-ledger.py post-finding \
  --repo <owner/repo> --pr <number> --head <full-head-sha> \
  --path <repository-relative-path> --line <right-side-line> \
...
```bash
python3 .agents/skills/critique/scripts/review-ledger.py reopen-occurrence \
...
```bash
python3 .agents/skills/critique/scripts/review-ledger.py dispose \
...
```bash
python3 .agents/skills/critique/scripts/review-ledger.py write-result \
...
```bash
python3 .agents/skills/critique/scripts/review-ledger.py write-blocked-result \
  --head "$(git rev-parse HEAD)" --engine "$AGENT_LOOP_REVIEW_ENGINE" \
  --round "$AGENT_LOOP_REVIEW_ROUND" --base "$AGENT_LOOP_REVIEW_BASE_SHA" \
  --before "$AGENT_LOOP_PR_HEAD_SHA" \
  --result-file "$AGENT_LOOP_REVIEW_RESULT_FILE" \
