...
> `review-ledger.js` is missing four invariants that the reference
...
Invoke it as `node .agents/skills/critique/scripts/review-ledger.js` — it
...
that enforces this runs in `gemini-platform`'s own CI, not here — in a consumer
...
Use `.agents/skills/critique/scripts/review-ledger.js` for every local-review
...
```bash
node .agents/skills/critique/scripts/review-ledger.js preflight-anchor \
  --repo <owner/repo> --pr <number> --head <full-head-sha> \
  --path <repository-relative-path> --line <right-side-line>

node .agents/skills/critique/scripts/review-ledger.js post-finding \
  --repo <owner/repo> --pr <number> --head <full-head-sha> \
  --path <repository-relative-path> --line <right-side-line> \
...
```bash
node .agents/skills/critique/scripts/review-ledger.js reopen-occurrence \
...
```bash
node .agents/skills/critique/scripts/review-ledger.js dispose \
...
```bash
node .agents/skills/critique/scripts/review-ledger.js write-blocked-result \
  --head "$(git rev-parse HEAD)" --engine "$AGENT_LOOP_REVIEW_ENGINE" \
  --round "$AGENT_LOOP_REVIEW_ROUND" --base "$AGENT_LOOP_REVIEW_BASE_SHA" \
  --before "$AGENT_LOOP_PR_HEAD_SHA" \
  --result-file "$AGENT_LOOP_REVIEW_RESULT_FILE" \
