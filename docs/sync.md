...
1. Add an entry to `scripts/sync-targets.yml` with `source`, `destination`, and `substitutions: []` (or the placeholder list). If an empty prose-only placeholder sits between blank separators, add that key to `collapse_empty_substitutions`; do not opt in keys used in literal content.
