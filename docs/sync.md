...
- **Template half (enforced here).** The engine renders faithfully: output is the template byte-for-byte with values spliced in. The single exception is a template line holding nothing but a placeholder that renders empty (`drop_empty_placeholder_lines` in `sync-engine.py`) — the line goes, and one adjacent blank line goes with it when keeping it would leave a blank-line run the author never wrote. Prettier collapses such a run, so without this every consumer's daily sync PR would fight the consumer's local format run. The `render-check` CI job renders the full surface against `tests/fixtures/render-check/.platform-config.yml` (which deliberately includes an empty value) and runs `prettier --check` on the result, then renders `tests/fixtures/render-fidelity/` and diffs it against a checked-in golden so a render that silently rewrites literal content fails too.
- **Consumer half (your `.platform-config.yml`).** The engine does not reformat your substitution values — tables, lists, and paragraphs inside them must already be prettier-clean markdown (e.g. a blank line between a paragraph and the list that follows it, aligned table pipes). This is a hard boundary, not a best effort: the engine has no Markdown parser and deliberately will not grow one, because anything that rewrites rendered output without knowing which bytes came from a value will corrupt literal content (prettier itself preserves blank runs inside indented code and raw `<pre>`). If a sync PR's diff against your repo is pure whitespace, your values are the first place to look.
