...
# target set ships two workflows, which every consumer receives and which
# are sensitive paths, so these entries are required; the refusal names any
# others that appear, in a block you can paste as-is.
