...
1. Add an entry to `scripts/sync-targets.yml` with `source`, `destination`, and `substitutions: []` (or the placeholder list). If an empty prose-only placeholder sits between blank separators, also add that key to `collapse_empty_substitutions` — it must appear in **both** lists, or the sync fails closed for every consumer. Do not opt in keys used in literal content; the `sync-targets` CI job checks both rules.
