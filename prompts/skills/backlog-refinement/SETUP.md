...
5. If `agent-loop-instructions.md` names a legacy rubric path, point it at `<<SKILLS_ROOT>>/backlog-refinement/core-rubric.md` and `.backlog/refinement.local.md` instead.
...
- Next steps: `<<INVOKE>>backlog-refinement queue`, then `refine --backfill` if the repository has issues assessed before this layer existed.
