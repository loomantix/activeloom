...
**Then ask what would have prevented it.** If the answer is a missing test seam, tangled callers, or hidden coupling, that is a cleanup with a scope of its own — file it, or hand it to `<<INVOKE>>refactorpass` on the follow-up PR with the specifics. Make that call **after** the fix is in; you know more now than when you started.
