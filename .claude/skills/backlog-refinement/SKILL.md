...
1. **Pre-check** the batch. This runs the mechanical checks once, deterministically, instead of in every assessor:
   ...
   ```bash
   python3 .claude/skills/backlog-refinement/scripts/precheck.py --unrefined --limit 25 --out <tmp>/precheck.json
   ```
...
3. **Review.** Merge the group files into one plan, then preview it:
   ...
   ```bash
   python3 .claude/skills/backlog-refinement/scripts/apply-plan.py <tmp>/plan.json
   ```
